...
from commu.model.config_helperCVAE import get_default_cfg_training
from commu.model.datasetCVAE import ComMUDatasetCVAE
...
def train():
    ...
    for batch, (data, target, meta, batch_token_num) in enumerate(
            train_real_iter
    ):
        ...
        meta_chunks = torch.chunk(meta, cfg.TRAIN.batch_chunk, 1)
...
dataset = ComMUDatasetCVAE(args.data_dir, cfg)
vocab = dataset.vocab
...
train_iter = dataset.get_iterator(
    batch_size, cfg.TRAIN.tgt_length, meta_len, device, "train", True, seed=local_seed
)
