...
class ComMU_Grouped_Dataset:
    ...
    def load_cache_data(self, dir_name, mode):
        if mode == "train":
            ...
            dat = []
            for i in range(len(data_input)):
                dat.append(self.group_data(data_input[i], data_target[i]))
        else:
            data_input = np.load(dir_name + '/input_val.npy', allow_pickle=True)
            data_target = np.load(dir_name + '/target_val.npy', allow_pickle=True)
            dat = []
    ...
    def group_data(self, data_input, data_target):
        bar_idx = np.cumsum(np.roll(data_target == 2, 1)) * 3
        chord_idx = np.cumsum(np.roll(np.logical_and(data_target >= 195, data_target <= 303), 1)) * 2
        idx = bar_idx + chord_idx + np.arange(np.shape(data_target)[0])
        total_length = idx[-1]
        ret = np.zeros((total_length // 4 + 12, 4), dtype=int)
        ret[idx // 4 + 11, idx % 4] = data_target
        ret[:11, 0] = data_input
        return ret
    ...
    def get_iterator(
        ...
        def iterator():
            ...
            tracker_list = [(i, 0) for i in range(batch_size)]
            next_idx = batch_size
            data = torch.LongTensor(bptt, batch_size, 4)
            target = torch.LongTensor(bptt, batch_size, 4)
            reset_mem = torch.BoolTensor(batch_size)
